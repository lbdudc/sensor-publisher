    ...
    <v-form v-model="validForm" ref="form">
      <v-row dense>
        ...
        <v-col cols="12" md="6">
          ...
          <v-row>
            ...
            <v-col cols="6">
              <autocomplete
                ...
                @change="fetchDataOnChange('Subdivisions1')"
            ...
            >
              <autocomplete
                ...
                @change="fetchDataOnChange('Subdivisions2')"
                @input="updateAddress()"
            ...
            >
              <autocomplete
                ...
                @change="fetchDataOnChange('Subdivisions3')"
                @input="updateAddress()"
            ...
            >
              <autocomplete
                ...
                @change="fetchDataOnChange('Subdivisions4')"
                @input="updateAddress()"
            ...
            >
              <autocomplete
                ...
                @change="fetchDataOnChange('Towns')"
                @input="updateAddress()"
          ...
          <v-btn @click="searchAddress()" width="100%" :disabled="!address.country">
...
export default {
  ...
  methods: {
    ...
    async fetchDataOnChange(child) {
      this.removeElement(child); // clearing children properties
      await this["fetch" + child](); //fetching child data
      this.$refs.form.validate(); // validate form on any change
    },
    ...
    searchAddress() {
      ...
      if (!formattedAddress.country.name)
        formattedAddress.country = this.countries.find(
          c => this.address.country.id === c.id
        );
      if (formattedAddress.subdivision1 && !formattedAddress.subdivision1.name)
        formattedAddress.subdivision1 = this.subdivisions1.find(
          s => this.address.subdivision1.id === s.id
        );
      if (formattedAddress.subdivision2 && !formattedAddress.subdivision2.name)
        formattedAddress.subdivision2 = this.subdivisions2.find(
          s => this.address.subdivision2.id === s.id
        );
      if (formattedAddress.subdivision3 && !formattedAddress.subdivision3.name)
        formattedAddress.subdivision3 = this.subdivisions3.find(
          s => this.address.subdivision3.id === s.id
        );
      if (formattedAddress.subdivision4 && !formattedAddress.subdivision4.name)
        formattedAddress.subdivision4 = this.subdivisions4.find(
          s => this.address.subdivision4.id === s.id
        );
      if (formattedAddress.town && !formattedAddress.town.name)
        formattedAddress.town = this.towns.find(
          t => this.address.towns.id === t.id
        );

      // call geolocate method from gc-map
      this.$refs[this.id].geolocate(formattedAddress);
    },
    fillLatLng(latLng) {
      if (latLng) {
        this.address.point = {
    ...
    fillAddress(results, latLng) {
      ...
      let search = this._setSearchResults(results); // fill address props with results
      this._searchResultsToGCAddress(search); // convert address props to GCAddress format
    ...
    _setSearchResults(results) {
      ...
      results.forEach(field => {
        if (field.types.includes("administrative_area_level_5"))
          ...
          search.subdivision4 = field;
        else if (field.types.includes("administrative_area_level_3") || field.types.includes("locality"))
          search.subdivision3 = field;
        else if (field.types.includes("administrative_area_level_2"))
          search.subdivision2 = field;
        else if (field.types.includes("administrative_area_level_1"))
          search.subdivision1 = field;
        else if (field.types.includes("country")) search.country = field;
        else if (field.types.includes("postal_code"))
          search.zipCode = field.long_name;
        else
          search.line1 = search.line1
            ? field.long_name.concat(", " + search.line1)
            : field.long_name;
      });

      return search;
    },
    async _searchResultsToGCAddress(search) {
      this.address.line1 = search.line1;
      this.address.zipCode = search.zipCode;
      this.address.country = this.countries.find(
        el =>
          el.name === search.country.long_name ||
          el.code === search.country.short_name
      );
      await this.fetchSubdivisions1();
      this.address.subdivision1 = this.subdivisions1.find(
        el =>
          el.name === search.subdivision1.long_name ||
          el.code === search.subdivision1.short_name
      );
        ...
        if (index === 4) {
          ...
          if (search.town) {
            let town = this.towns.find(
              el =>
                el.name === search.town.long_name ||
                el.code === search.town.short_name
            );
            this.address.town = town ? town : null;
          } else this.address.town = null;
        } else {
          await this["fetchSubdivisions" + index]();
          if (search["subdivision" + index])
            this.address["subdivision" + index] = this[
              "subdivisions" + index
            ].find(
              el =>
                el.name === search["subdivision" + index].long_name ||
                el.code === search["subdivision" + index].short_name
            );
          else break;
        }
      }
