...
export default {
  ...
  computed: {
    calcHeaders() {
      const calcCategories = CALCS.map((el) => {
        return {
          text: `${el.key} ${this.$t(
            this.propertyUnits[this.categoryAggSelected.toLowerCase()]
          )}`,
    ...
    calcItems() {
      // show only the measurements that are closer to the current CALCS.key
      ...
      CALCS.forEach((calc) => {
        const data = measurements
          ...
          .filter((el) => parseInt(el.id) <= parseInt(calc.key));
      ...
      // change the items to be an array of objects with the following structure:
      // Create a row for each propertyUnits , getting the value from the data

      const itemsTable = [];
      Object.keys(this.propertyUnits)
        .filter(
          (property) =>
            property.toLowerCase() !== this.categoryAggSelected.toLowerCase()
        )
        .forEach((property) => {
          const row = {
            [this.categoryAggSelected.toLowerCase()]: property.toLowerCase(),
          };

          items.forEach((item) => {
            row[item.id] = item.value[property.toLowerCase()];
          });

          row["check"] = false;
          row["cat_value"] = items[0].catValue;
