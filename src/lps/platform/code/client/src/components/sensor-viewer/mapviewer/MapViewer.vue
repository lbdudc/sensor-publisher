  ...
  <div id="layer-manager">
    ...
    <div id="map-container">
      ...
      /*% if (feature.SV_Popup) { %*/
       ...
       <information-popup-category
        v-if="isCategorized && storeOptionsRequest"
        ref="informationPopupCat"
        :propertySelected="
...
import InformationPopup from "@/components/sensor-viewer/common/components/InformationPopup.vue";
  /*% if (hasCategoricalDims) { %*/
import InformationPopupCategory from "@/components/sensor-viewer/common/components/InformationPopupCategory.vue";
  /*% } %*/
/*% } %*/
...
export default {
  ...
  computed: {
    ...
    isCategorized() {
      return (
        this.store?.getSelector(AGGREGATIONS.CATEGORY)?.items != null &&
        this.store?.getSelector(AGGREGATIONS.CATEGORY)?.items.length > 0
      );
    },
    useElastic() {
  ...
  methods: {
    async loadMap() {
      ...
      if (this.store.objFromObservable[FLAGS.REAL_TIME]) {
        this.onRealTimeCheckboxChange(true);
      } else {
    ...
    _geoJSONPopupFunction(form) {
      return (layer) => {
        this._getFeatureData(layer);
        this.popupForm = form;
        this.popupLayer = layer;
        return this.$refs.informationPopup.$el;
      };
    },
    _geoJSONPopupFunctionCat(form) {
      return (layer) => {
        this._getFeatureData(layer).then((res) => {
          layer.feature.properties.measurements = res;
          this.popupForm = form;
          this.popupLayer = layer;
        });
        return this.$refs.informationPopupCat.$el;
      };
    },
    /*% } %*/
    _getFeatureData(layer) {
      ...
      return this.dataRepository
        ...
        .then((res) => {
          if (this.isCategorized) return filterResults(res, this.store, true);
          else return filterResults(res.data, this.store, false);
          this.noDataDialog = layer.feature.properties.data == null;
        })
        .finally(() => {
    ...
    async onRealTimeCheckboxChange(val) {
      const isActive = this.store.getSelector(FLAGS.REAL_TIME).value;
      await this.store.setSelector(FLAGS.REAL_TIME, val, false, !isActive);
      if (val) {
        if (this.intervalId == null) {
          await triggerInstantGetter(this.store);
          this.intervalId = setInterval(triggerInstantGetter, 300000);
        }
      } else {
