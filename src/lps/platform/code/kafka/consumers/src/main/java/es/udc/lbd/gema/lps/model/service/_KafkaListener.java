...
public class /*%= normalize(context.id, true) %*/KafkaReader {
  ...
  @KafkaListener(
      id = "/*%= normalize(context.id, false).toLowerCase() %*/-/*%= normalize(context.datasource, false) %*/-consumer",
      topics = "/*%= normalize(context.id, false).toLowerCase() %*/",
      groupId = "/*%= normalize(context.id, false).toLowerCase() %*/-/*%= normalize(context.datasource, false) %*/-consumer",
      autoStartup = "true")
  public void consume(
