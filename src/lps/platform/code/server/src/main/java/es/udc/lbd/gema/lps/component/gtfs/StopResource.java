...
import es.udc.lbd.gema.lps.component.gtfs.model.service.StopService;
...
@RestController
@RequestMapping(StopResource.STOPS_RESOURCE_URL)
public class StopResource {
  public static final String STOPS_RESOURCE_URL = "/api/gtfs/stop";

  private static final Logger logger = LoggerFactory.getLogger(StopResource.class);

  @Inject private StopService stopService;
   ...
   */
  @GetMapping
  public ResponseEntity<Page<StopDTO>> getAll(
      @PageableDefault(page = 0, size = 100000, sort = "id") Pageable pageable,
      @RequestParam(value = "filters", required = false) List<String> filters,
      @RequestParam(value = "search", required = false) String search) {
    Page<StopDTO> page = stopService.getAll(pageable, filters, search);
    HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(page, STOPS_RESOURCE_URL);
    return new ResponseEntity<>(page, headers, HttpStatus.OK);
  }

  @GetMapping("/geom/stopLoc")
  public ResponseEntity<FeatureCollectionJSON> getStopLoc(
      @RequestParam(value = "properties", defaultValue = "false", required = false)
          Boolean properties,
      @RequestParam(value = "filters", required = false) List<String> filters) {

    FeatureCollectionJSON featureCollection = stopService.getStopLoc(properties, filters);
    return new ResponseEntity<>(featureCollection, HttpStatus.OK);
  }

  @GetMapping("/{id}")
  public ResponseEntity<StopFullDTO> get(@PathVariable String id) {
    try {
      return new ResponseEntity<>(stopService.get(id), HttpStatus.OK);
  ...
  @PostMapping
  public ResponseEntity<?> create(@Valid @RequestBody StopFullDTO stop, Errors errors)
    ...
    try {
      StopFullDTO result = stopService.create(stop);
      return ResponseEntity.created(
              new URI(String.format("%s/%s", STOPS_RESOURCE_URL, result.getStopId())))
          .body(result);
  ...
  public ResponseEntity<?> update(
      @PathVariable String id, @Valid @RequestBody StopFullDTO stop, Errors errors) {
    if (errors.hasErrors()) {
      return ResponseEntity.badRequest().body(ValidationErrorUtils.getValidationErrors(errors));
    }
    try {
      StopFullDTO result = stopService.update(id, stop);
