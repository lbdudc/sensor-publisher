...
/*%
    var pkName = normalize(context.properties.filter(function(p) { return p.pk; })[0].name);
%*/
...
public /*% if (context.abstract) { %*/abstract /*% } %*/class /*%= normalize(context.name, true) %*/ /*% if (context.superclass) { %*/extends /*%= normalize(context.superclass, true) %*/
  /*% } else if (context.traceable) { %*/extends AbstractOwnerEntity /*% } %*/{
            ...
            /*% if (prop.multiple) { %*/
                /*% if (relationshipIsManyInOtherDirection) { %*/
                    /*% if (prop.owner) {
                        ...
                        var auxIdRelationship = getIdProperty(relationship.class).name;
                    %*/
    ...
    @Column(name = "/*%= camelToSnake(normalize(prop.name)) %*/"/*%= extraColumn %*/ /*% if (prop.unique) { %*/ ,unique=true/*% } %*/)
        /*% } %*/
