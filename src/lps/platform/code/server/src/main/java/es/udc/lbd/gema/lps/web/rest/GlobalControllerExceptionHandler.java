...
public class GlobalControllerExceptionHandler {
  ...
  public @ResponseBody ResponseEntity<Void> methodArgumentTypeMismatchException(
    ...
    public @ResponseBody ResponseEntity<Void> badCredentialsExceptionHandler(CredentialsAreNotValidException e) {
        logger.error(e.getMessage(), e);
        return ResponseEntity.status(HttpStatus.UNAUTHORIZED).headers(HeaderUtil.createError("account.credentials_not_valid", null))
                .build();
    }
    /*% } %*/
    ...
    public @ResponseBody ResponseEntity<Void> ExpiredJwtExceptionHandler(ExpiredJwtException e) {
        logger.error(e.getMessage(), e);
        return ResponseEntity.status(HttpStatus.UNAUTHORIZED).headers(HeaderUtil.createError("account.expired_JWT", null))
                .build();
    }
    /*% } %*/
