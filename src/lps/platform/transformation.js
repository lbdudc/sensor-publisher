...
function _getProperties(e, entities, forms=[], superclassId="") {
  const properties = forms.length ? _getPropertiesLists(e, entities, forms, superclassId) : _getPropertiesForms(e, entities, superclassId);

  const superclass = e.superclass ? entities.find((entity) => entity.name === e.superclass) : null;
  const superclassIdProperty =  superclass?.properties.find((p) => p.pk);

  return superclass ? properties.concat(_getProperties(superclass, entities, forms, superclassIdProperty.name)) : properties;
}
...
function _getPropertiesLists(e, entities, forms, superclassId) {
  return e.properties
      ...
      .filter((p) => p.class !== "GCAddress" && p.class !== "IGGallery" && p.name !== superclassId)
      .map((p) => {
        return {
          property: p.name,
